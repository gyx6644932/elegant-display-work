...
if Request.form("Oper") <> "" then
	...
	IF strLength(title) <= 0 then jsouterr("服务项目名称不能为空，且不能太长！")
	IF strLength(title) > 24 then jsouterr("服务项目名称不能为空，且不能太长！")
	IF strLength(content) > 140 then jsouterr("服务项目说明太长，请简化！")
	...
	sql = "select * from Web_fuwu where id = "&id
